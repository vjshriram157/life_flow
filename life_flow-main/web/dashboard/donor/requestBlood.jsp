...
<div class="d-flex">
    ...
    <div class="container-fluid p-4 p-md-5 w-100">
        ...
        <%
            // 🎯 OPTIMIZATION: Fetch all approved blood banks for the dropdown
            // Impact: N reads where N is the number of banks. Very safe for daily limits.
            List<QueryDocumentSnapshot> allBanks = new java.util.ArrayList<>();
            try {
                Firestore dbBanks = FirebaseConfig.getFirestore();
                allBanks = dbBanks.collection("blood_banks").whereEqualTo("status", "APPROVED").get().get().getDocuments();
            } catch (Exception e) {}
        %>
                ...
                <div class="card card-modern h-100">
                    <div class="card-body p-4 p-md-5">
                        <form action="<%=request.getContextPath()%>/api/peer-request" method="post" id="requestForm">
                            ...
                            <div class="mb-4">
                                <label class="form-label text-white-50 text-uppercase fw-bold" style="font-size: 0.75rem; letter-spacing: 1px;">Target Blood Bank / Hospital</label>
                                <div class="input-group">
                                    ...
                                    <select id="hospitalName" class="form-select bg-dark text-white border-secondary border-opacity-25 py-2" required onchange="updateCity(this)">
                                        <option value="" disabled selected>Choose a Registered Bank...</option>
                                        <% for(QueryDocumentSnapshot bank : allBanks) { %>
                                            <option value="<%= bank.getString("bank_name") %>" data-city="<%= bank.getString("city") %>" data-id="<%= bank.getId() %>">
                                                <%= bank.getString("bank_name") %>
                                            </option>
                                        <% } %>
                                    </select>
                                    <input type="hidden" name="bankId" id="bankIdHidden">
                                </div>
                            </div>
                            
                            <div class="row g-4 mb-4">
                                <div class="col-md-6">
                                    <label class="form-label text-white-50 text-uppercase fw-bold" style="font-size: 0.75rem; letter-spacing: 1px;">City</label>
                                    <input type="text" id="city" class="form-control bg-dark text-white border-secondary border-opacity-25 py-2" placeholder="Where is blood needed?" required readonly>
                                    <div class="form-text text-white-50" style="font-size: 0.65rem;">Auto-detected from bank selection</div>
                                </div>
                                <div class="col-md-6">
                    ...
                    <div class="card card-modern bg-dark border-secondary border-opacity-25 rounded-4">
                        <div class="card-body p-3 position-relative overflow-hidden">
                            ...
                            <div class="mt-3 text-end">
                                <form action="<%=request.getContextPath()%>/api/peer-request" method="post" class="m-0">
                                    <input type="hidden" name="action" value="complete">
                                    <input type="hidden" name="redirect" value="requestBlood">
                                    <input type="hidden" name="requestId" value="<%= doc.getId() %>">
                                    <button type="submit" class="btn btn-outline-success btn-sm rounded-pill px-3 py-1" style="font-size: 0.75rem;">
...
<script>
    function updateCity(select) {
        ...
        document.getElementById('bankIdHidden').value = bankId || "";
    }
    ...
    document.getElementById('requestForm').addEventListener('submit', function(e) {
        ...
        document.getElementById('hospitalCityCombined').value = hospital + ", " + city;
    });
</script>
